...
int main(int argc, char** argv){
    ...
    if(rank==0){
        ...
        max_parciales[0]=maxNumero(numeros, block_size);
        min_parciales[0]= minNumero(numeros,block_size);
        for (int i = 1; i < nprocs; ++i) {
        ...
        std::printf("\n");
        int max_parcial= maxNumero(numeros, block_size);
        std::printf("Max parcial=%d,",max_parcial);
        std::printf("\n");
        MPI_Send(&max_parcial, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);

        int min_parcial= minNumero(numeros,block_size);
