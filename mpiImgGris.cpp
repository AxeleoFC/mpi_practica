...
std::vector<sf::Uint8> grisag(const sf::Uint8 * img, int width, int height){
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            int intesidad= img[index]*0.299+img[index+1]+img[index+2]*0.144;
            buffer[index] = intesidad;
            buffer[index + 1] = intesidad;
            buffer[index + 2] = intesidad;
            buffer[index + 3] = 255;
        }
    }
    return buffer;
...
int main(int argc, char** argv) {
    ...
    int filas_r;
    std::vector<sf::Uint8> imagenGris;
    ...
    if (rank == 0) {
        ...
        imagenGris.resize(width * height * rgba);
    }
    ...
    std::vector<sf::Uint8> gris= grisag(buffer.data(), width, filas_por_proceso);

    MPI_Gather(gris.data(), pixeles_por_fila * filas_r, MPI_UNSIGNED_CHAR,
               imagenGris.data(), pixeles_por_fila * filas_r, MPI_UNSIGNED_CHAR,
               0, MPI_COMM_WORLD);

    MPI_Finalize();
    if(rank==0){
        abrirImg(image, imagenGris.data());
    }
