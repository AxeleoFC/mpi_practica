...
std::tuple<sf::Uint8, sf::Uint8, sf::Uint8> calculate_median(const sf::Uint8 *image, int width, int height, int x, int y) {
    ...
    std::sort(rojo.begin(), rojo.end());
    std::sort(verde.begin(), verde.end());
    std::sort(azul.begin(), azul.end());

    int mediana=rojo.size()/2;

    return std::make_tuple(rojo[mediana], verde[mediana], azul[mediana]);
}

std::vector<sf::Uint8> medianaImg(const sf::Uint8* img, int width, int height) {
    std::vector<sf::Uint8> buffer(width * height * rgba);
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            auto [r, g, b] = calculate_median(img, width, height, i,j);
            int index = (j * width + i) * rgba;
            buffer[index] = r;
            buffer[index + 1] = g;
            buffer[index + 2] = b;
...
int main(int argc, char** argv){
    ...
    std::vector<sf::Uint8> maxParci= medianaImg(buffer.data(),width,filas_r);
