...
std::tuple<sf::Uint8, sf::Uint8, sf::Uint8> calcular_min(const sf::Uint8 *image, int width, int height, int x, int y) {
    ...
    auto max_rojo = *std::min_element(rojo.begin(), rojo.end());
    auto max_verde = *std::min_element(verde.begin(), verde.end());
    auto max_azul = *std::min_element(azul.begin(), azul.end());

    return std::make_tuple(max_rojo, max_verde, max_azul);
}

std::vector<sf::Uint8> minImg(const sf::Uint8* img, int width, int height) {
    std::vector<sf::Uint8> buffer(width * height * rgba);
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            auto [r, g, b] = calcular_min(img, width, height, i,j);
            int index = (j * width + i) * rgba;
            buffer[index] = r;
            buffer[index + 1] = g;
            buffer[index + 2] = b;
...
int main(int argc, char** argv){
    ...
    std::vector<sf::Uint8> maxParci= minImg(buffer.data(),width,filas_r);
