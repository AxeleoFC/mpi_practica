...
std::vector<sf::Uint8> multiImagenes(const sf::Uint8 * img1, const sf::Uint8 * img2, int width, int height){
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            buffer[index] = (img1[index]*img2[index])/255;
            buffer[index + 1] = (img1[index+1]*img2[index+1])/255;
            buffer[index + 2] = (img1[index+2]*img2[index+2])/255;
            buffer[index + 3] = 255;
        }
    }
    return buffer;
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        imagenM.resize(width * height * rgba);
    }
    ...
    std::vector<sf::Uint8> multPar= multiImagenes(buffer1.data(), buffer2.data(), width, filas_por_proceso);

    MPI_Gather(multPar.data(), pixeles_por_fila * filas_por_proceso, MPI_UNSIGNED_CHAR,
               imagenM.data(), pixeles_por_fila * filas_por_proceso, MPI_UNSIGNED_CHAR,
               0, MPI_COMM_WORLD);

    MPI_Finalize();
    if(rank==0){
        abrirImg(image1, imagenM.data());
    }
