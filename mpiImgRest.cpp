...
std::vector<sf::Uint8> restaImagenes(const sf::Uint8 * img1, const sf::Uint8 * img2, int width, int height){

    std::vector<sf::Uint8> buffer(width * height * rgba);
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            int index = (i * width + j) * rgba;
            buffer[index] = abs(img1[index]-img2[index]);
            buffer[index + 1] = abs(img1[index+1]-img2[index+1]);
            buffer[index + 2] = abs(img1[index+2]-img2[index+2]);
            buffer[index + 3] = 255;
        }
    }
    return buffer;
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        imagenR.resize(width * height * rgba);
    }
    ...
    fmt::println("Filas por proceso: {}, inicio_filas: {}, fin_filas: {}", (fin_filas-inicio_filas), inicio_filas, fin_filas);
    ...
    std::vector<sf::Uint8> restaP= restaImagenes(buffer1.data(), buffer2.data(), width, filas_por_proceso);


    MPI_Gather(restaP.data(), pixeles_por_fila * filas_por_proceso, MPI_UNSIGNED_CHAR,
               imagenR.data(), pixeles_por_fila * filas_por_proceso, MPI_UNSIGNED_CHAR,
               0, MPI_COMM_WORLD);

    MPI_Finalize();
    if(rank==0){
        abrirImg(image1, imagenR.data());
    }
